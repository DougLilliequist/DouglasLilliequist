...
float dither8x8(vec2 position, float brightness) {
  ...
  //set domain of pixels to be within the 8x8 matrix dimensions
  int x = int(mod(position.x, 8.0));
  int y = int(mod(position.y, 8.0));

  //get value in matrix / array
  int index = x + y * 8;

  //the limit which will be used to determine wether or not a pixel will be white or black
  float limit = 0.0;
